...
public class SetService {
	...
	public void ex1() {
		...
		//->boolean 반환 : 제거되면 true
		set.remove("직방");
		
		System.out.println(set/*.toString()*/);
		
		//Set에 저장된 요소 하나씩 얻어오기
		//1) Iterator(=반복자) : 컬레션에서 제공하는 컬렉션 객체 반복 접근자
		//-> 컬렉션에 저장된 객체를 임의의 순서로 하나씩 꺼내는 역할
		Iterator<String> it=set.iterator();
		//Set.iterator() : Set에 저장된 객체를 이용해서 Iterator 객체를 만들어 반환
		while(it.hasNext()) { //다음 값이 존재하면 true
			String temp=it.next(); //next() : 다음 값을 얻어 옴
	...
	public void ex2() {
		 ...
		 * == equals() 또는 hashCode()를 오버라이딩하면 나머지 하나도 반드시 오버라이딩 해라
		 * */
		Set<Student> set=new HashSet<Student>();
		set.add(new Student("홍길동",15,"서울시 중구",'M',100));
		set.add(new Student("김길순",18,"서울시 종로구",'F',90));
		set.add(new Student("홍길동",15,"서울시 중구",'M',100));
		
		Student s1=new Student("김길순",18,"서울시 종로구",'F',90);
		Student s2=new Student("김길순",18,"서울시 종로구",'F',90);
	...
	public void ex3() {
		...
		Integer i1=10; 
		//Integer = int 
		//Integer = new Intger(int) 로 컴파일러가 바꿔줌
