...
public class UserView {
	...
	//User 참조 변수 선언(참조하는 객체가 없다)
	private User user=null; //회원 가입 정보를 담고 있는 User 객체 참조
	
	private User loginUser=null; //로그인 한 User 객체의 주소를 참조힐 변수
	//null==로그인X, nullX==로그인O
	...
	public void signUp() {
		...
		//회원 가입 서비스 호출
		user=service.signUp(id, pw, name, age, gender); //User 객체 주소 반환
		//->필드에 존재하는 user 참조 변수에 반환된 User 객체 주소 저장
	...
	public void login() {
		...
		if(loginUser != null) {
			...
			System.out.print("비밀번호 : ");
			String pw=sc.next();
			int result = service.login(id, pw, user); //로그인 서비스 호출 (반환 값:-1,0,1)
			if(result==-1) {
				...
				loginUser=user; //user 변수가 참조하고 있는 객체의 주소를 loginUser에 대입(얕은 복사)
			}
		}
	}
	...
	//로그아웃 메서드
	public void logout() {
		if(loginUser!=null) { //로그인이 되어 있는 상태
			loginUser=null;
	...
	public void printUser( ) {
		...
		if(loginUser!=null) { //로그인O
			//loginUser가 참조하는 객체에서 getter를 이용해 필드 값을 얻어 와 출력
