...
public class AbstractionService {
	...
	public void ex1() {
		//추상 클래스는 정말로 객체 생성이 안될까 : O
//		Animal a=new Animal();  //Cannot instantiate the type Animal
			
		//Animal을 상속받은 자식 클래스를 이용해 객체 생성
		Fish f1=new Fish();
		f1.setType("어류");  //자식 객체 내부에 부모 부분이 있음 확인
//		f1.breath();
		
		Person p1=new Person();
		p1.setType("포유류"); //자식 객체 내부에 부모 부분이 있음 확인
		
		//추상 클래스는 부모 타입의 참조 변수로 사용할 수 있다 (다형성의 업 캐스팅)
		Animal a1=new Fish();
		...
		a1.breath();
		a2.breath();
		//실행 전 Animal.breath()와 연결 : 정적 바인딩
		//실행 중 오버라이딩 한 자식 breath()로 연결 : 동적 바인딩 
		//동적 바인딩에 의해 정상 실행될 것 -> 에러 안 나는 이유
		...
		//arr[i] 인덱스에 어떤 Animal 자식 객체가 있는지 모르지만 강제 오버라이딩한 breath()라는 공통 메서드 소유
		//->arr[i].breath() 작성 가능
	...
	public void ex2() {
		//인터페이스는 미완성 메서드로만 이루어져서 직접 객체 생성 불가
//		KH k1=new KH(); //Cannot instantiate the type KH
		...
		arr[1]=new HGD(); //업캐스팅
		//부모 타입 참조변수 = 자식 객체
		//부모가 KH 인터페이스  -> 인터페이스도 부모 참조 변수로 사용 가능
	...
	public void ex3() {
		...
		System.out.printf("%d * %d = %d\n",num1,num2,cal.multiple(num1, num2));
		if(num2==0) {
