...
//		String input=br.readLine(); //== sc.nextLine();
		...
		//->readLine() 메서드는 IOException을 던질 가능성이 있기 때문에 호출 시 반드시 예외처리 -->Checked Exception
		...
		//보통 개발자의 실수로 발생하는 예외로 필수적으로 처리하지 않아도 된다 --> Unchecked Exception
	}
	...
	public void ex2() {
		...
		try { 
			...
			//예외 강제 발생 == 예외 객체 생성해서 던짐
			throw new IOException();
			
		} catch (IOException e) {
			//catch : try 구문 내에서 예외 객체가 던져진 경우 매개변수(참조형)을 이용해서 해당 객체를 잡음(주소 저장해서 참조)
	...
	public void ex3() {
		...
		} catch(ArithmeticException e) { //catch문 다중 작성 가능
										 //catch문이 여러 개면 위에서부터 순서대로 발생한 예외를 검사하여
										 //알맞은 매개변수를 가진 catch에서 처리한다.
			//산술적 예외(Unchecked)
			System.out.println("0으로 나눌 수 없습니다.");
		} catch(InputMismatchException e) { 
			//스캐너를 이용한 입력 시 데이터 타입이 올바르지 않으면 발생하는 예외
			//ex) sc.nextIn(); 를 통한 입력 시 정수가 아닌 값을 입력하면 발생
			System.out.println("정수만 입력해주세요.");
		} catch(Exception e){
			System.out.println("예외가 발생했습니다.");
		} finally {
			//finally : catch 구문 수행 여부와 관계없이 무조건 실행하는 구문
			System.out.println("프로그램 종료");
			// 자원 반환 구문을 주도 finally에 작성
			sc.close();
		} 
		//*Exception 클래스
		//-모든 예외 관련 클래스의 최상위 "부모"
		
		//*예외 발생 == throw new 예외 클래스(); == 예외 객체가 생성되어 던져짐
	...
	public void ex6() throws IOException {
		//해당 메서드에서 발생하는 IOException을 호출한 메서드로 던져버림(책임 전가)
		throw new IOException();
	}
